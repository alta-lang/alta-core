...
ALTACORE_AST_VALIDATE_D(Type) {
  ...
  if (info->type->isFunction) {
    if (info->type->isAny) ALTACORE_VALIDATION_ERROR("type can't be multiple kinds, only one of `function`, `any`, or `native`");
    ...
    if (info->type->isAny) {
      ...
      for (auto& mod: info->type->modifiers) {
